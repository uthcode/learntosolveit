...
The RPN notation for assigning to variables is like this `10 A =`. When an `=` sign is encountered the previous value is
popped and the value that is stored in `var` variable (that is the previous one is taken) and then it's value is assigned
to the next popped variable. Thus two variables `A` and `B` are set in the above expression.
