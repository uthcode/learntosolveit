...
Write a program to print a histogram of the lengths of words in its input.  Draw the
histogram with the bars vertical.
...
Next we use the WORD array above and we go about printing from left to right.
We we start left, we can see if our left position (j) is <=  WORD value in that position, WORD[j].
This is accomplished in the for loop `if j <= word[j]` if it is, then we print
* otherwise we print space ''.
...
We will keep going for this till we get j == 4 and at that moment, our output will be::
