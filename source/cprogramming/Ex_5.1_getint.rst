...
We are to explain the function `getint(int *)` which takes a pointer to an integer as the argument. We also use `getch` and `ungetch` as two functions, from chapter 4, which work on `buf` of `BUFSIZE` sharing a global variable called `bufp`.
`ungetch` function returns the character read to `buf` while `getch` tries to read that character to our program and if no character is present, it uses `getchar` to get the character.
...
Ensures that if we get a character which is not +,-, digit, EOF, then we return 0 and in the main loop we end the program. 
That is, we strictly look for characters that can be converted to integer in this program. So the only valid inputs are like this.
...
This is responsible for converting the character like `1` to integer 1 and store it in *pn, the place in the array.
We multiply the number by sign and when we find EOF, we store that EOF, so that the program terminates correctly.
