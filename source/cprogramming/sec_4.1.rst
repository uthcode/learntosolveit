...
Here we have the source string in **s** and target string in **t**. We start
taking each character in s and starting at the position of the character, we
check if the entire target string **t** is present in the string.

The checking for the entire target is present is done by the second for loop and
the if statement.::
...
In the first for loop we use a temperorary variable k to iterate through and
check if the target string **t** is present in **s**. If the target string is
entirely present, which is ensured by if statement checking for `\0`, we return
the position **i**.
