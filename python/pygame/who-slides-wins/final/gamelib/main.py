...
def main():
    ...
    headingfont = pygame.font.Font(fontfile,20)
    headingfont.set_underline(True)
    resultfont = pygame.font.Font(fontfile,30)
    compplayertext = playersfont.render("Computer Window",0,FONTCOLOR)
    playertext = headingfont.render("Player Window",0,FONTCOLOR)
    ...
    text6 = messagefont.render("Player Turns:",0,FONTCOLOR)
    text7 = messagefont.render("ENTER - when finished!",0,FONTCOLOR)
    text8 = messagefont.render("Q - Quit",0,FONTCOLOR)
    ...
    game_surface.blit(text6,(20,480))
    game_surface.blit(text7,(20,520))
    game_surface.blit(text8,(20,540))
    ...
    playermsg = "You took: %d steps" % (playersteps /2)
    pygame.display.update(game_surface.get_rect())
    playerresult = messagefont.render(playermsg, 0, FONTCOLOR)
    game_surface.blit(playerresult, (500, 130))
    if pygame.mixer:
        music = os.path.join('data','computer.wav')
        pygame.mixer.music.load(music)
        pygame.mixer.music.play(-1)
    for step in solution:
        compplay.reorder(step)
        compplay.update()
        game_surface.blit(cp_surface, (3,3))
    computersteps = "Computer: %d steps" % (len(solution) - 1)
    computerresult = messagefont.render(computersteps,0,FONTCOLOR)
    game_surface.blit(computerresult,(240,130))
    pygame.time.wait(200)
    if pygame.mixer:
        music = os.path.join('data','end.wav')
        pygame.mixer.music.load(music)
        pygame.mixer.music.play(-1)
    while True:
        if (len(solution)-1) < (playersteps/2):
            msg = "Computer Won!"
        elif (len(solution)-1) > (playersteps/2):
            msg = "You Won!"
        else:
            msg = "Its a Draw."
        result = resultfont.render(msg, 0, FONTCOLOR)
        game_surface.blit(result,(380,100))
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                sys.exit()
            elif event.type == KEYDOWN or event.type == KEYUP:
